locals {
  ...
  sendgrid_records = {
    "em1788" = "u3848296.wl047.sendgrid.net."
    "s1._domainkey" = "s1.domainkey.u3848296.wl047.sendgrid.net."
    "s2._domainkey" = "s2.domainkey.u3848296.wl047.sendgrid.net."
  }
}
...
resource "digitalocean_record" "varified_workspace" {
  domain = var.domain
  type   = "TXT"
  name   = "@"
  value  = "google-site-verification=guiOUcxdeMIuveFP0MvLSkzqEtIzAefLc6DtPCy_J5s"
}
...
resource "digitalocean_domain" "public_domain" {
  name       = var.domain
  ip_address = var.ip_server
}

data "digitalocean_domain" "domian" {
  name = var.domain
}
